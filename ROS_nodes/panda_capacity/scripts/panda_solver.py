...
# 
# dof, output space
n = len(kdl_kin.joint_limits_lower)
# output space
m = 3

# https://frankaemika.github.io/docs/control_parameters.html
# maximal torques
t_max =  np.array([kdl_kin.joint_limits_effort]).T #np.array([[87], [87], [87], [87], [12], [12], [12]]) #np.array([[1], [1], [1], [1], [1], [1], [1]])
t_min = -t_max
# maximal joint velocities
dq_max = np.array([kdl_kin.joint_limits_velocity]).T
dq_min = -dq_max
# maximal joint angles
q_max = np.array([kdl_kin.joint_limits_upper]).T
q_min = np.array([kdl_kin.joint_limits_lower]).T
# matrix of axis vectors - for polytope search
T_vec = np.diagflat(t_max-t_min)
# matrix of 
T_min = np.matlib.repmat(t_min,1,2**m)


# intersection
t_max_int =  np.vstack((t_max,t_max))
t_min_int = -t_max_int
T_vec_int = np.diagflat((t_max-t_min,t_max-t_min))
T_min_int = np.matlib.repmat(t_min_int,1,2**m)


# direct kinematics functions for 7dof
def forward(q):
    return kdl_kin.forward(q)

def dk_position(q):
    return forward(q)[:3,3]

def dk_orientation_matrix(q):
    return forward(q)[0:3,0:3]

def jacobian(q):
    Jac = kdl_kin.jacobian(q)
    return Jac

def jacobian_position(q):
    return jacobian(q)[:3, :]

def jacobian_pseudo_inv(q):
    return np.linalg.pinv(jacobian(q))
...
def manipulability_velocity(q, direction = None):
    ...
    if direction is None:
        ...
        Jac = np.array(direction).dot(jacobian_position(q))
    # limits scaling
...
def manipulability_force(q, direction = None):
    # avoid 0 angles
    joint_pos = np.array(q)
    joint_pos[joint_pos==0] = 10**-7
...
def max_force_in_direction(q, dir):

    # jacobian calculation
    Jac = np.array(dir).transpose()*jacobian_position(q)
    m = 1
    # calculate svd
    ...
    f_vertex = J_n_invT*( t_vertex- gravity_torque(q) )
...
# maximal end effector force
def force_polytope_auctus(q, direction = None, force = None):

    # jacobian calculation
    Jac_full = jacobian_position(q)
